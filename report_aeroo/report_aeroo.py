...
class Aeroo_report(report_sxw):
    ...
    def create_source_pdf(self, cr, uid, ids, data, report_xml, context=None):
        # este es un patch para imprimir tres veces los remitos y que ponga
        # origina, duplicado y triplicado.
        # Se requiere setear el numero de copias en 3 en el documento
        # correspondiente de aeroo y agregar las keys print_original,
        # print_duplicado y print_triplicado.
        # ademas se debe modificar el odt para agregar las tres palabras con
        # un condicional que seteamos aqui.
        #import wdb;wdb.set_trace()

        domain = [[('name', '=', 'print_original')],
                  [('name', '=', 'print_duplicado')],
                  [('name', '=', 'print_triplicado')]]

        values = [[True, False, False],
                  [False, True, False],
                  [False, False, True]]

        def set_text(lines, domain, copy, value):
            original = lines.search(domain[0])
            duplicado = lines.search(domain[1])
            triplicado = lines.search(domain[2])
            if original:
                original.value_boolean = value[0]
            if duplicado:
                duplicado.value_boolean = value[1]
            if triplicado:
                triplicado.value_boolean = value[2]

        output = PdfFileWriter()
        lines = report_xml.line_ids

        for copy in range(report_xml.copies):
            set_text(lines, domain, copy, values[copy])

            a, b = self._create_source_pdf(cr, uid, ids, data, report_xml,
                                           context)
